...
export default function FloatingPostButton({
  ...
  return (
    <button
      ...
      } right-3 sm:right-6 w-11 h-11 sm:w-14 sm:h-14 rounded-full shadow-md sm:shadow-lg transition-all duration-300 z-[100] ${
        disabled
    ...
    >
      ...
      {/* Decorative only — never steal clicks from the real button */}
      {!disabled && (
        <div
          className="pointer-events-none absolute inset-0 rounded-full bg-primary-400 dark:bg-primary-300 opacity-0 animate-ping"
          aria-hidden
        />
      )}
    </button>
