  ...
  try {
    ...
    const trendingTags = Object.values(tagStats)
      .map((tag) => ({
        ...tag,
        growth_rate:
          tag.usage_count > 0 ? (tag.recent_usage / tag.usage_count) * 100 : 0,
      }))
      .sort((a, b) => {
    ...
    const emojiResult = await query<{ emoji: string; count: string }>(
      `
    ...
    const peakHoursResult = await query<{ hour: number; count: string }>(
      `
    ...
    const ventsTodayResult = await query<{ count: string }>(
      `SELECT COUNT(*)::int AS count FROM vents WHERE created_at >= $1`,
      [today.toISOString()]
    )

    const reactionsTodayResult = await query<{ count: string }>(
