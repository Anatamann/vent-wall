...
export async function checkRegistrationAllowed(
  ...
  const blocked = await query(
    `SELECT reason FROM blocked_ip_hashes
     WHERE ip_hash = $1
...
export async function recordRegistrationAttempt(
  ...
  await query(
    `INSERT INTO registration_attempts (id, ip_hash, succeeded)
     VALUES ($1, $2, $3)`,
    [attemptId, ipHash, succeeded]
  )
}

export async function blockIpHash(
  ipHash: string,
  reason: string,
  blockHours?: number
): Promise<void> {
  ...
  await query(
    `INSERT INTO blocked_ip_hashes (ip_hash, reason, expires_at)
     VALUES ($1, $2, $3)
