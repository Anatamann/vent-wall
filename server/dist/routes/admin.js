...
router.get('/overview', async (_req, res) => {
    try {
        ...
        const [ventsToday, reactionsToday, commentsToday, gifCommentsToday, newUsersToday, newFeedback, topTags, topEmojis,] = await Promise.all([
            query(`SELECT COUNT(*)::int AS count FROM vents WHERE created_at >= $1`, [today.toISOString()]),
            query(`SELECT COUNT(*)::int AS count FROM reactions WHERE created_at >= $1`, [today.toISOString()]),
            query(`SELECT COUNT(*)::int AS count FROM vent_comments WHERE created_at >= $1`, [today.toISOString()]),
            query(`SELECT COUNT(*)::int AS count FROM vent_comments
         WHERE comment_type = 'gif' AND created_at >= $1`, [today.toISOString()]),
            query(`SELECT COUNT(*)::int AS count FROM users WHERE created_at >= $1`, [today.toISOString()]),
            query(`SELECT COUNT(*)::int AS count FROM user_feedback WHERE status = 'new'`),
            query(`
...
router.get('/feedback', async (req, res) => {
    try {
        ...
        const conditions = statusFilter === 'all' ? '' : `WHERE f.status = $1`;
        const params = statusFilter === 'all' ? [perPage, offset] : [statusFilter, perPage, offset];
        const limitOffsetParams = statusFilter === 'all' ? '$1 OFFSET $2' : '$2 OFFSET $3';
      ...
      `, params);
        const countResult = await query(statusFilter === 'all'
            ? `SELECT COUNT(*)::int AS count FROM user_feedback`
            : `SELECT COUNT(*)::int AS count FROM user_feedback WHERE status = $1`, statusFilter === 'all' ? [] : [statusFilter]);
        return res.json({
            items: result.rows,
            page,
            per_page: perPage,
...
router.patch('/feedback/:id', async (req, res) => {
    ...
    try {
        const existing = await query(`SELECT id FROM user_feedback WHERE id = $1`, [req.params.id]);
