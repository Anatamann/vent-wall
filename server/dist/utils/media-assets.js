...
async function secureDownload(remoteUrl) {
...
async function downloadToCache(remoteUrl, mimeType) {
...
export async function fetchAllowedGifBuffer(remoteUrl, maxBytes) {
    if (!isAllowedDownloadUrl(remoteUrl)) {
        throw new Error('GIF URL is not from an allowed provider');
    }
    const response = await secureDownload(remoteUrl);
...
export async function findKlipyCachedFilePath(externalId) {
    const result = await query(`SELECT file_path FROM media_assets
...
export async function findReusableKlipyAsset(externalId) {
    const result = await query(`SELECT * FROM media_assets
     WHERE source = 'klipy' AND external_id = $1 AND expires_at > now()
     ORDER BY created_at DESC
     LIMIT 1`, [externalId]);
    return result.rows[0] ?? null;
}
async function extendAssetExpiry(assetId, expiresAt) {
    const result = await query(`UPDATE media_assets
     SET expires_at = GREATEST(expires_at, $2)
     WHERE id = $1
     RETURNING *`, [assetId, expiresAt.toISOString()]);
...
export async function ingestKlipyGif(options) {
    ...
    try {
        ...
        const inserted = await query(`INSERT INTO media_assets (
...
export async function getMediaAsset(assetId) {
    const result = await query('SELECT * FROM media_assets WHERE id = $1', [assetId]);
    return result.rows[0] ?? null;
}
export function resolveMediaAbsolutePath(relativePath) {
...
export function openMediaReadStream(relativePath) {
    return createReadStream(resolveMediaAbsolutePath(relativePath));
}
export async function cleanupExpiredMediaAssets() {
    const expired = await query(`SELECT * FROM media_assets WHERE expires_at < now()`);
    let removed = 0;
