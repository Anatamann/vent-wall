...
async function seed() {
    ...
    for (const user of USERS) {
        await query(`INSERT INTO users (id, username, email, password_hash, last_post_date, post_count_today)
       VALUES ($1, $2, $3, $4, CURRENT_DATE, 0)`, [user.id, user.username, user.email, passwordHash]);
    }
    for (const tag of MOOD_TAGS) {
        await query(`INSERT INTO mood_tags (id, name, color, emoji) VALUES ($1, $2, $3, $4)`, [tag.id, tag.name, tag.color, tag.emoji]);
    }
    ...
    for (const vent of VENTS) {
       ...
       VALUES ($1, $2, $3, $4, $5)`, [vent.id, vent.user_id, vent.content, createdAtIso, expiresAt.toISOString()]);
        for (const tagName of vent.tags) {
            await query('INSERT INTO vent_tags (vent_id, tag_id) VALUES ($1, $2)', [vent.id, tagByName[tagName]]);
        }
    }
    const reactions = [
    ...
    for (const reaction of reactions) {
        await query('INSERT INTO reactions (vent_id, user_id, emoji) VALUES ($1, $2, $3)', [reaction.vent_id, reaction.user_id, reaction.emoji]);
    }
